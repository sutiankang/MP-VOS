...
def get_scheduler(optimizer, n_iter_per_epoch, args):
    if "cosine" in args.scheduler:
        scheduler = CosineAnnealingLR(
            optimizer=optimizer,
            eta_min=0.000001,
            T_max=(args.epochs - args.warmup_epochs) * n_iter_per_epoch)
    elif "lr_step" in args.scheduler:
        ...
        scheduler = MultiStepLR(
            ...
            milestones=[(m - args.warmup_epochs) * n_iter_per_epoch for m in lr_decay_epochs])
    else:
    ...
    if args.warmup_epochs > 0:
        scheduler = GradualWarmupScheduler(
            ...
            warmup_epoch=args.warmup_epochs * n_iter_per_epoch)
    return scheduler